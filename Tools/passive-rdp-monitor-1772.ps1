...
$text = Replace-Once $text 'private AxHost AxHost => (AxHost)Control;' @'
...
'@ @'
        ...
        private sealed class PassiveRdpInputBlocker : IMessageFilter
        {
            ...
            public bool PreFilterMessage(ref Message m)
            {
                ...
                foreach (var control in _blockedControls.ToArray())
                {
...
$text = Replace-Once $text 'RdpClient8.Reconnect((uint)size.Width, (uint)size.Height);' @'
RdpClient8.Reconnect((uint)size.Width, (uint)size.Height);
                ScrollToLowerRightAsync();
'@ 'RdpProtocol8 reconnect scroll'

$text = Replace-Once $text '_controlBeginningSize = Size.Empty;' @'
_controlBeginningSize = Size.Empty;
            ScrollToLowerRightAsync();
'@ 'RdpProtocol8 resize scroll'
